...
const SearchBar = ({ params, handleChangeIsSearching }: SearchBarProps) => {
  ...
  const currencyFiltering =
    navigation.location &&
    new URLSearchParams(navigation.location.search).has("currency");
  const typeFiltering =
    navigation.location &&
    new URLSearchParams(navigation.location.search).has("type");

  useEffect(() => {
    if (!searching || !currencyFiltering || !typeFiltering) {
      handleChangeIsSearching(false);
    } else {
      handleChangeIsSearching(true);
    }
  }, [searching, currencyFiltering, typeFiltering, handleChangeIsSearching]);
    ...
    <search>
      ...
      >
        <div className="flex flex-col md:flex-row">
          ...
          >
            <option value="">{searchBar.filters.currencies.mainOption}</option>
            {Object.entries(searchBar.filters.currencies.options).map(
              ([key, value]) => (
          ...
          >
            <option value="">{searchBar.filters.types.mainOption}</option>
            {Object.entries(searchBar.filters.types.options).map(
              ([key, value]) => (
                <option key={key} value={key}>
                  {value as string}
                </option>
          ...
          <div className="relative w-full">
            ...
            <div
              role="status"
              aria-hidden="true"
              hidden={!searching || !currencyFiltering || !typeFiltering}
